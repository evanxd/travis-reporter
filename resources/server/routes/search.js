...
router.get('/', function(req,res){
	repoter.find({},null,{sort: {'errCount': -1}}).exec(function(err,repoter)
	{
...
router.get('/crate/:fileName/:errorCount/:date', function(req,res){
	new repoter({
    	...
    	errCount : req.params.errorCount,
...
router.get('/range/:range_start/:range_end', function(req,res){
	var	result = repoter.find({},null, {sort: {"errCount": -1}});
		result = result.where('errCount').lte(req.params.range_end).gte(req.params.range_start);
	result.exec(function(err,result)
	{
		res.send(JSON.parse(JSON.stringify(result)));
		//console.dir(JSON.parse(JSON.stringify(result)));
